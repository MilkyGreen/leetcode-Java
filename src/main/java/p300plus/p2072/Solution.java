...
public class Solution {
    ...
    public double[] dicesProbability(int n) {
        ...
        dfs(n, 0);
        double[] ans = new double[sums.size()];
        int i = 0;
        for (Map.Entry<Integer, Integer> entry : sums.entrySet()) {
            ans[i] = entry.getValue() / (double) total;
            i++;
        }
        return ans;
    }

    private void dfs(int leftN, int sum) {
        if (leftN == 0) {
            sums.put(sum, sums.getOrDefault(sum, 0) + 1);
            return;
        }
        for (int i = 0; i < nums.length; i++) {
            dfs(leftN - 1, sum + nums[i]);
        }
    }

    public static void main(String[] args) {
        new Solution().dicesProbability(10);
    }
